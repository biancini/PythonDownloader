...
class TwitterRestPager(object):
  ...
  def get_iterator(self, wait=5, new_tweets=False):
    ...
    while True:
      ...
      iterator = req.get_iterator()
      if new_tweets:
        iterator.results = reversed(iterator.results)
      ...
      # use the first or last tweet item_id to limit (depending on the newer/older direction)
      # the next request
      if item_id is None:
        continue
      elif new_tweets:
        self.params['since_id'] = str(item_id)
      else:
        self.params['max_id'] = str(item_id - 1)
